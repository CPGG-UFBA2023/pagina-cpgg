    ...
    email_slug := regexp_replace(
      regexp_replace(
        lower(translate(NEW.name,
          'áàâãäåāăąçćčñéèêëēĕėęěíìîïĩīĭįóòôõöōŏőúùûüũūŭůűýÿÁÀÂÃÄÅĀĂĄÇĆČÑÉÈÊËĒĔĖĘĚÍÌÎÏĨĪĬĮÓÒÔÕÖŌŎŐÚÙÛÜŨŪŬŮŰÝŸ',
          'aaaaaaaaacccneeeeeeeeeiiiiiiiiooooooooouuuuuuuuuyyaaaaaaaaacccneeeeeeeeeiiiiiiiiooooooooouuuuuuuuuyy')),
        '[^a-z0-9]+', '.', 'g'),
      '(^\.|\.$)', '', 'g'
    );
